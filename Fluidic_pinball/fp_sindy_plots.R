...
preds_n10 = readRDS("Results/sindy_preds_n10.RData")
preds_n20 = readRDS("Results/sindy_preds_n20.RData")
preds_n30 = readRDS("Results/sindy_preds_n30.RData")
nb_time_steps = 50
coordinates = c('Cl', 'Cd')

#for (i in 1:500) {
#  for (j in 1:2) {
#    preds_n30[[i]][[j]]$yh = preds_n30[[i]][[j]]$pred_smooth
#    preds_n30[[i]][[j]]$Y = preds_n30[[i]][[j]]$real
#    preds_n30[[i]][[j]] <- subset(preds_n30[[i]][[j]], select = -c(pred, real, pred_smooth))
#  }
#}
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    ...
    NRMSE = sqrt(mean((preds_nn[[i]][[j]]$Y[1:nb_time_steps] - preds_nn[[i]][[j]]$yh[1:nb_time_steps])^2))/obs_sd
    NRMSEs_nn[i, j] = NRMSE
  }
}
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    ...
    NRMSE = sqrt(mean((preds_n10[[i]][[j]]$Y[1:nb_time_steps] - preds_n10[[i]][[j]]$yh[1:nb_time_steps])^2))/obs_sd
...
vars_n10 = apply(NRMSEs_n10, 2, var) 
...
for(j in 1:nb_series) {
  ...
  for(i in 1:nb_time_steps) {
    pred_aux = matrix(0, nb_trials, 2)
    for(k in 1:nb_trials) {
      pred_aux[k, 1] = preds_n10[[k]][[j]]$Y[i] 
      pred_aux[k, 2] = preds_n10[[k]][[j]]$yh[i]
    }
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    ...
    NRMSE = sqrt(mean((preds_n20[[i]][[j]]$Y[1:nb_time_steps] - preds_n20[[i]][[j]]$yh[1:nb_time_steps])^2))/obs_sd
...
vars_n20 = apply(NRMSEs_n20, 2, var)  
...
for(j in 1:nb_series) {
  ...
  for(i in 1:nb_time_steps) {
    pred_aux = matrix(0, nb_trials, 2)
    for(k in 1:nb_trials) {
      pred_aux[k, 1] = preds_n20[[k]][[j]]$Y[i] 
      pred_aux[k, 2] = preds_n20[[k]][[j]]$yh[i]
    }
...
for(i in 1:nb_trials) {
  for(j in 1:nb_series) {
    ...
    NRMSE = sqrt(mean((preds_n30[[i]][[j]]$Y[1:nb_time_steps] - preds_n30[[i]][[j]]$yh[1:nb_time_steps])^2))/obs_sd
...
vars_n30 = apply(NRMSEs_n30, 2, var) 
...
for(j in 1:nb_series) {
  ...
  for(i in 1:nb_time_steps) {
    pred_aux = matrix(0, nb_trials, 2)
    for(k in 1:nb_trials) {
      pred_aux[k, 1] = preds_n30[[k]][[j]]$Y[i] 
      pred_aux[k, 2] = preds_n30[[k]][[j]]$yh[i]
    }
